...
func Find(x float64, withOutside bool, sort Check, data ...Point) (y float64, err error) {
	defer func() {
		if err != nil {
			err = errors.Join(fmt.Errorf("graph.Find"), err)
		}
	}()
// }
`,
...
`,
	...
	filenames := [2]string{"find.go", "approx.go"}
	var body [2]string
	for i := range mtx {
		if len(mtx) == 0 {
			panic("zero mtx")
	...
	for i := range filenames {
		err := os.WriteFile(filenames[i], []byte(body[i]), 0644)
		if err != nil {
			panic(err)
		}
	}
	...
	for i := range filenames {
		err := exec.Command("goimports", "-w", filenames[i]).Run()
		if err != nil {
			panic(err)
		}
	}
